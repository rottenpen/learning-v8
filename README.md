...
```
Function::Call 可以在 `src/api/api.cc` 里面找到: 
```c++
...
chain? 
函数被调用时，异常是怎么在调用链里通报（reported）/触发（triggered）的？
...
```
Root 是 `src/object/visitors.h` 里的一个 enum。这个 enum is generated by a macro
and expands to:
```c++
enum class Root {                                                               
  kStringTable,
